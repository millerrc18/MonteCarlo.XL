...
public partial class SetupViewModel : ObservableObject
{
    ...
    /// <summary>
    /// Loads a saved SimulationProfile into the SetupViewModel, recreating
    /// InputCardViewModels and OutputCardViewModels from the saved data.
    /// Called by the Addin wiring layer when a saved config exists.
    /// </summary>
    public void LoadFromProfile(MonteCarlo.Engine.Simulation.SimulationProfile profile)
    {
        // Clear existing
        Inputs.Clear();
        Outputs.Clear();
        ...
        IsSeedLocked = profile.RandomSeed.HasValue;

        foreach (var savedInput in profile.Inputs)
        {
            try
            {
                var parameters = new Dictionary<string, double>(savedInput.Parameters);
                var dist = DistributionFactory.Create(savedInput.DistributionName, parameters);
                var card = new InputCardViewModel(
                    savedInput.SheetName,
                    savedInput.CellAddress,
                    savedInput.Label,
                    savedInput.DistributionName,
                    parameters,
                    dist);
                Inputs.Add(card);
            }
            ...
            {
                // Skip inputs that can't be recreated (e.g., invalid params)
            }
        }

        foreach (var savedOutput in profile.Outputs)
        {
            var card = new OutputCardViewModel(
                savedOutput.SheetName,
                savedOutput.CellAddress,
                savedOutput.Label);
            Outputs.Add(card);
        }

        // Restore correlation matrix if present
        CorrelationMatrixValues = profile.CorrelationMatrix;
    }
