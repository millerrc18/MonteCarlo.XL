...
public class AddIn : IExcelAddIn
{
    ...
    /// <summary>Pipeline wiring — connects UI events to Excel/orchestrator.</summary>
    internal static SimulationPipelineWiring? Wiring { get; private set; }
    ...
    public void AutoOpen()
    {
        try
        {
            ...
            // Subscribe to task pane creation to wire the simulation pipeline
            TaskPane.TaskPaneCreated += OnTaskPaneCreated;
    ...
    /// <summary>
    /// Called when the task pane host is first created.
    /// Wires the simulation pipeline between UI and Excel.
    /// </summary>
    private static void OnTaskPaneCreated(TaskPane.TaskPaneHost host)
    {
        if (Orchestrator == null || InputTags == null || OutputTags == null ||
            Highlighter == null || ConfigPersistence == null || Workbook == null)
            return;

        Wiring?.Dispose();
        Wiring = new SimulationPipelineWiring(
            host.MainViewModel,
            Orchestrator,
            InputTags,
            OutputTags,
        ...
        // Auto-load saved config into the SetupViewModel
        Wiring.LoadSavedProfile();
    }
