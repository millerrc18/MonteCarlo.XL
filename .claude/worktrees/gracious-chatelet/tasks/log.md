...
## END-TO-END WIRING — Full Simulation Pipeline Integration
**Status**: COMPLETE
**Date**: 2026-03-24
**Branch**: proto/gracious-chatelet

### What Was Done

#### 1. Core Pipeline Wiring (SimulationPipelineWiring.cs — new, ~500 lines)
Created the central wiring class that connects all UI events to the Addin layer:
- **RunSimulationRequested** → syncs TagManagers from SetupViewModel → saves config → calls `SimulationOrchestrator.RunSimulationAsync()`
- **CancelSimulationRequested** → calls `SimulationOrchestrator.CancelSimulation()`
- **Orchestrator.ProgressChanged** → marshals to UI thread → `MainViewModel.OnSimulationProgress()`
- **Orchestrator.SimulationComplete** → marshals to UI thread → `MainViewModel.OnSimulationComplete()` with pre-computed stats and sensitivity data
- **Orchestrator.SimulationError** → marshals to UI thread → `MainViewModel.OnSimulationError()`
- All orchestrator→UI callbacks use `Application.Current.Dispatcher.BeginInvoke()` for thread safety

#### 2. Cell Selection Wiring
- **SetupViewModel.CellSelectionRequested** → hooks Excel's `Worksheet.SelectionChange` event
- Captures sheet name, cell address, and auto-detects a suggested label from adjacent cells (left or above)
- One-shot: unhooks after first selection
- Calls `SetupViewModel.OnCellSelected()` on UI thread

#### 3. Input/Output Tagging & Highlighting
- **SetupViewModel.InputAdded** → `InputTagManager.TagInput()` + `CellHighlighter.HighlightInput()`
- **SetupViewModel.InputRemoved** → `InputTagManager.UntagInput()` + `CellHighlighter.ClearHighlight()`
- **SetupViewModel.OutputAdded** → `OutputTagManager.TagOutput()` + `CellHighlighter.HighlightOutput()`
- **SetupViewModel.OutputRemoved** → `OutputTagManager.UntagOutput()` + `CellHighlighter.ClearHighlight()`
- Before each simulation run, tag managers are re-synced from SetupViewModel state

#### 4. Correlation Editor Navigation
- **SetupViewModel.CorrelationEditorRequested** → creates `CorrelationViewModel`, initializes with current input labels and saved matrix → navigates `MainViewModel.CurrentView` to `CorrelationView`
- **CorrelationViewModel.Applied** → saves matrix back to `SetupViewModel.CorrelationMatrixValues`
- **CorrelationViewModel.CloseRequested** → navigates back to SetupView

#### 5. Config Auto-Load
- Added `SetupViewModel.LoadFromProfile(SimulationProfile)` method that recreates InputCardViewModels and OutputCardViewModels from saved data
- On first task pane creation, `SimulationPipelineWiring.LoadSavedProfile()` loads from ConfigPersistence and populates the UI
- Re-tags and highlights cells in Excel after restoration

#### 6. Debounced Auto-Save
- After any input/output add/remove, schedules a config save with 2-second debounce
- Calls `SimulationOrchestrator.SaveConfig()` with current iteration count and seed

#### 7. Results → Tornado Chart Integration
- Added `SensitivityResults` and `BaseOutputValue` properties to `ResultsViewModel`
- Updated `RecomputeStats()` to populate sensitivity data when switching outputs (uses pre-computed data from orchestrator, falls back to on-demand computation)
- Added `TornadoChart` control to `ResultsView.xaml` below the histogram/CDF area
- Tornado chart is bound to `SensitivityResults` and `BaseOutputValue`

#### 8. Export Button Wiring
- Added `ExportRequested` event and `ExportToSheetCommand` to `ResultsViewModel`
- Wired in `SimulationPipelineWiring`: renders histogram and tornado charts as PNG, calls `ResultsExporter.ExportSummary()` with rendered images
- Bound "Export to Sheet" button in ResultsView.xaml to `ExportToSheetCommand`

#### 9. Ribbon & Keyboard Shortcuts
- `OnRunSimulation` now triggers simulation via `SetupViewModel.RunSimulationCommand` instead of just showing the task pane
- `OnStopSimulation` calls `Orchestrator.CancelSimulation()` (was TODO)
- `MonteCarloRunSimulation` keyboard shortcut (Ctrl+Shift+R) triggers simulation if inputs are configured

#### 10. Thread Marshaling
- All orchestrator→UI callbacks dispatched via `Application.Current.Dispatcher.BeginInvoke()`
- Verified `Application.Current` is the one created by `TaskPaneHost.EnsureWpfApplication()`

### Files Created
- `src/MonteCarlo.Addin/Services/SimulationPipelineWiring.cs` — central event wiring (~500 lines)

### Files Modified
- `src/MonteCarlo.Addin/AddIn.cs` — added Wiring property, OnTaskPaneCreated handler, cleanup in AutoClose
- `src/MonteCarlo.Addin/TaskPane/TaskPaneHost.cs` — exposed MainViewModel property
- `src/MonteCarlo.Addin/TaskPane/TaskPaneController.cs` — added Host property, TaskPaneCreated event
- `src/MonteCarlo.Addin/MonteCarloRibbon.cs` — wired Run/Stop buttons to orchestrator
- `src/MonteCarlo.Addin/KeyboardShortcuts.cs` — Ctrl+Shift+R triggers simulation
- `src/MonteCarlo.UI/ViewModels/MainViewModel.cs` — updated OnSimulationComplete signature to pass stats/sensitivity
- `src/MonteCarlo.UI/ViewModels/SetupViewModel.cs` — added LoadFromProfile method
- `src/MonteCarlo.UI/ViewModels/ResultsViewModel.cs` — added sensitivity properties, ExportRequested event, ExportToSheetCommand, pre-computed stats caching
- `src/MonteCarlo.UI/Views/ResultsView.xaml` — added TornadoChart, wired Export button

### Key Decisions Made During Implementation
- **Lazy wiring**: The pipeline wiring is created on first task pane open (via TaskPaneCreated event), not at AutoOpen(). This avoids creating WPF objects before the task pane is needed.
- **Sync before run**: TagManagers are re-synced from SetupViewModel before each simulation run, ensuring they always match the UI state.
- **Pre-computed data passthrough**: Stats and sensitivity data computed by the orchestrator are passed through to ResultsViewModel to avoid duplicate computation.
- **Object initialization**: CellReference uses `required` properties (not constructor), so all instantiation uses object initializers.
- **Action disambiguation**: In the Addin project, `Action` conflicts between `System.Action` and `Microsoft.Office.Interop.Excel.Action`. Used fully-qualified `System.Action` in the wiring class.

### Issues / Notes for Architect
- **Runtime testing not possible**: No Excel environment available. All code follows established patterns from the codebase and should work correctly.
- **ConvergenceUpdated event**: Still unused (CS0067 warning). Could be wired if live convergence data is desired during simulation.
- **Correlation matrix**: Wired through to CorrelationView but not yet applied to the SimulationConfig during runs. The Iman-Conover engine exists (TASK-015/016) but the orchestrator doesn't call it yet with the matrix from SetupViewModel.
- **Chart rendering for export**: Uses WPF RenderTargetBitmap which requires the control to be measured/arranged. This may need a STA thread if called from a background thread.

### Build Results
```
Build succeeded.
    1 Warning(s) [CS0067 — ConvergenceUpdated unused, expected]
    0 Error(s)
```
