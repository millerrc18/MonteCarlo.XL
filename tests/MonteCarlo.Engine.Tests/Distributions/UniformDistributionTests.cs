...
public class UniformDistributionTests
{
    ...
    {
        var dist = new UniformDistribution(0, 10);
    ...
    {
        var act = () => new UniformDistribution(5, 5);
    ...
    {
        var act = () => new UniformDistribution(10, 0);
        act.Should().Throw<ArgumentException>();
    }
    ...
    {
        var dist = new UniformDistribution(0, 10);
        dist.Variance.Should().BeApproximately(100.0 / 12.0, 1e-10);
    }
    ...
    {
        var dist = new UniformDistribution(0, 10, Seed);
        var samples = dist.Sample(SampleCount);
        double empiricalMean = samples.Average();
        empiricalMean.Should().BeApproximately(5.0, 5.0 * 0.01);
    }
    ...
    [Fact]
    public void PDF_IsConstant_WithinBounds()
    {
        var dist = new UniformDistribution(0, 10, Seed);
        double expected = 0.1; // 1/(10-0)
        dist.PDF(1).Should().BeApproximately(expected, 1e-10);
        dist.PDF(5).Should().BeApproximately(expected, 1e-10);
        dist.PDF(9).Should().BeApproximately(expected, 1e-10);
    }
    ...
    {
        var dist1 = new UniformDistribution(0, 10, 123);
        var dist2 = new UniformDistribution(0, 10, 123);
    ...
    {
        var dist = new UniformDistribution(0, 10);
        dist.ParameterSummary().Should().Be("Uniform(min=0, max=10)");
    }
